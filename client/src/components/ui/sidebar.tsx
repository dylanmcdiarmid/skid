...
// Sidebar skeleton width generation (percentage values)
...
type CookieStoreWindow = Window & { cookieStore: CookieStoreLike };

function hasCookieStore(w: Window): w is CookieStoreWindow {
  return "cookieStore" in w;
}
...
function SidebarProvider({
  ...
  const setOpen = useCallback(
    (nextOpen: boolean | ((prevOpen: boolean) => boolean)) => {
      ...
      if (typeof window !== "undefined" && hasCookieStore(window)) {
        window.cookieStore
          .set({
...
function SidebarMenuSkeleton({
  ...
  const width = useMemo(() => {
    return `${
      Math.floor(Math.random() * SKELETON_WIDTH_RANGE_PERCENT) +
      SKELETON_WIDTH_MIN_PERCENT
    }%`;
  }, []);
